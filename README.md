...
<link href="https://hanzi-bank.vercel.app/">
...
- **app/** — Next.js App Router 기반 페이지 및 기능별 컴포넌트
- **(auth)** - 인증 관련 페이지 및 컴포넌트
- **(main)** - 메인 서비스 페이지(급수별 단어, 마이페이지, 검색)
- **components/ui** — 여러 페이지에서 공통으로 쓰이는 UI 컴포넌트
- **hooks/** — 커스텀 훅 (서버 상태 연동, 편집 상태 관리 등)
- **lib/** — 순수 유틸 함수 (포맷팅, URL 변환 등)
